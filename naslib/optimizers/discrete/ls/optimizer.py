...
class LocalSearch(MetaOptimizer):
    ...
    def get_nbhd(self, chosen):
        ...
        for i, cell in enumerate(cells):
            ...
            for edge in edges:
                ...
                nbr_cells[i].edges[edge].set('op_index', 1, shared=True)     # this is search space dependent
    ...
    def new_epoch(self, epoch):
        ...
        if epoch < self.num_init:
            ...
            if len(self.nbhd) == 0:
                if not self.chosen or self.best_arch.accuracy > self.chosen.accuracy:
                    self.chosen = self.best_arch
                else:
                    self.chosen = sample_random_architecture(self.search_space, self.scope)
